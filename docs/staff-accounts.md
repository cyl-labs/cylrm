...
- `app_user` (username, name, scrypt hash, role `admin`/`closer`/`caller`, active — **closer** added 2026-09-25, see `docs/meetings.md` "Closers") and a nullable `call.user_id`. Nullable and unbackfilled on purpose: the calls made before this existed belong to nobody, and the stats show them as "Not attributed" rather than inventing an owner. Schema in `2026-08-13-app-user.sql`.
...
- **`ADMIN_ONLY_CALL_PREFIXES` is Team, Payroll and Spend**, kept separate from `EMAIL_PREFIXES` so the two reasons stay legible — one is a different product, the other is a permission — with `isAdminOnlyPath` covering both for the middleware and `linksFor` dropping them from the caller's sidebar. `/call-stats` was on it until 2026-09-06; see below.
- **`/call-stats` is two screens sharing one page** (2026-09-06). An admin gets the floor: everyone's calls, every niche, a person picker, a By-person table. A caller gets their own and nothing else, and the page is the whole control — `mine = me?.role !== "admin"` forces `personId` to themselves and passes `scopeId` to `getCallLists`/`getListStats` for the niches they may see. **Anything added to that screen has to take one or the other**, or it will quietly show a caller the floor.
  ...
  - It was closed to callers entirely until then, which cost the wrong thing. With the Scoreboard shut too (2026-09-03), the people doing the dialling had no way to see their own day, hear a call back, or check the pickup count they are paid on — none of which is anybody else's business to protect. The Scoreboard was admin-only from 2026-09-03 and **reopened to callers on 2026-09-24** at the founders' request: it was closed while one person was dialling, since a leaderboard of one reads as a thin operation, and there is now a floor for it to rank. It shows call counts, never pay.
