...
7. **Spreadsheet** (Calling) — every calling lead in one grid, built to read as a spreadsheet: column letters, frozen header, row numbers, a formula bar showing the selected cell, arrow-key/Tab cell selection, Ctrl+C to copy a cell, and a tab per call list along the bottom. Category is a column and also where one is corrected, so a mis-tapped outcome is fixed in place. A category filter and a search box narrow the sheet; Export CSV downloads exactly what is filtered.
