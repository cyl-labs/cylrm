...
- **The calling board's stages are derived, never stored** — the same rule as everywhere else here. To call / Tried (no answer, voicemail, gatekeeper) / Call back / Interested / Demo booked, read off the latest call. Moving a card therefore *is* logging the call that moved it; there is no status to set on its own. "To call" accepts no drops, because nothing you can do on a phone makes a lead never-rung again. Not-interested and bad-number leads are counted, not carried — the board is the work in front of you.
- **Calling metrics are their own definitions.** A *pickup* is a call where somebody answered: gatekeeper, callback, not interested, interested, demo booked. Voicemail and no answer are not, and neither is a bad number — the line was wrong and nobody was spoken to, so counting it would flatter the pickup rate exactly where the data is worst. Calls are attempts; interested and demos are counted as distinct leads, because logging "interested" twice for one business is still one interested business.
...
9. **Call stats** (Calling) — calls logged, leads dialled, pickups, interested and demos over a date range, an outcome breakdown, calls per day for the last fortnight with the pickup share shaded in, and a per-list table pairing lifetime progress (leads, worked) with range performance (calls, pickups, interested, demos).
