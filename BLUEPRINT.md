...
- **Two ways to look at a list.** Dial works one number at a time and is the default; Sheet is the whole list as a spreadsheet, in company order, filterable by category. They answer different questions — "who do I ring next" and "what is on this list" — so the sheet deliberately does not inherit the queue's ordering.
...
6. **Call lists** (Calling) — cards per list with worked/never-called progress, positive outcomes, and a badge for callbacks that are due. CSV import needs only a phone column. Opening a list is the dialler: one lead at a time with a full-width button that copies the number, a notes pad, and outcome buttons grouped into "didn't connect" and "done with them"; Queue / Callbacks / Closed / All views; skip without logging leaves a lead in place for later. A **Sheet** toggle beside those views swaps the dialler for the whole list as a spreadsheet — sticky header, row numbers, one row per lead — with category chips carrying counts, a search box, per-row category correction, and a CSV export of whatever is on screen.
