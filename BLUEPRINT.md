...
- **A lead's state is derived from its latest call, never stored.** There is no status column to fall out of step; correcting a mis-tapped outcome is just logging again. Outcomes split into ones that keep a lead in the cold-calling queue (`no_answer`, `voicemail`, `callback`) and ones that take it out (`demo_booked`, `trial`, `won`, `lost`, `not_interested`, `bad_number`) — you stop cold-calling a business the moment it has a demo in the diary. The set of outcomes plus "never called" is what the UI calls a lead's **category**.
- **There is no `gatekeeper`.** Reaching a receptionist rather than the owner is the same fact as not reaching anyone — `no_answer` already says it, and a third shade of "did not get through" only made the outcome buttons harder to pick from at speed. The four rows that existed became `no_answer`.
...
- **The calling board's stages are derived, never stored** — the same rule as everywhere else here. To call / Tried (no answer, voicemail) / Call back / Demo booked / Trial / Won / Lost, read off the latest call. Moving a card therefore *is* logging the call that moved it; there is no status to set on its own. "To call" accepts no drops, because nothing you can do on a phone makes a lead never-rung again. Lost holds all three ways of ending with no sale — refused on the phone, a number that was never theirs, and a trial that did not convert — because on a board they mean the same thing.
- **Calling metrics are their own definitions.** A *pickup* is a call where somebody answered: callback, not interested, demo booked, trial, won, lost. Voicemail and no answer are not, and neither is a bad number — the line was wrong and nobody was spoken to, so counting it would flatter the pickup rate exactly where the data is worst. Calls are attempts; demos, trials and wins are counted as distinct leads, because logging "demo booked" twice for one business is still one demo.
...
6. **Call lists** (Calling) — cards per list with worked/never-called progress, positive outcomes, and a badge for callbacks that are due. CSV import needs only a phone column. Opening a list is the dialler: one lead at a time with a full-width button that copies the number, a notes pad, and outcome buttons grouped into "didn't connect" and "done with them", picked first and written only when the confirm button below them is pressed; Queue / Callbacks / Closed / All views; skip without logging leaves a lead in place for later. Below the card, **every** lead still to work in that view is listed, not the first handful — a tab called "All" that showed six of forty was not showing all — and tapping one dials it next, so the fortieth lead does not cost thirty-nine skips.
7. **Spreadsheet** (Calling) — every calling lead in one grid, built to read as a spreadsheet: column letters, frozen header, row numbers, a formula bar showing the selected cell, arrow-key/Tab cell selection, Ctrl+C to copy a cell, and a tab per call list along the bottom. Clicking a number cell copies the number — the same clipboard-not-`tel:` move the dialler makes — and says "Copied" in the cell rather than raising a toast per number. **The lead's own fields are editable in place** (Company, Phone, Contact, Title, Email) via an Edit button on the formula bar, Enter/F2, or a double-click, because a scraped number is wrong often enough to need fixing where it is seen; a corrected number is re-keyed so dedupe keeps working, is refused if it cannot be rung in Singapore or already belongs to another lead on that list, and the draft stays open when it is refused. The derived columns are not editable — Category has its dropdown, and Tries / Last call / Callback / Notes are fixed by correcting the call. Each list also links to its own tab from its call list screen. Category is a column and also where one is corrected, so a mis-tapped outcome is fixed in place. A category filter and a search box narrow the sheet; Export CSV downloads exactly what is filtered. The category cell's menu logs a call at the top level and corrects the last one a level in.
8. **Pipeline** (Calling) — the calling board: To call / Tried / Call back / Interested / Demo booked, every card a lead with a copy-to-clipboard number, its list, and how many tries it has had. Drag or use the card's menu to log a call; the menu carries every outcome, including the two that close a lead out and so have no column. Columns show at most 60 cards and say how many they are holding back — a list of a thousand uncalled numbers is a queue, and the dialler is where it gets worked.
...
9. **Call stats** (Calling) — calls logged, leads dialled, pickups, demos, trials and wins over a date range (Won also shows its share of everything decided, won against lost), an outcome breakdown, calls per day for the last fortnight with the pickup share shaded in, and a per-list table pairing lifetime progress (leads, worked) with range performance (calls, pickups, demos, trials, won).
