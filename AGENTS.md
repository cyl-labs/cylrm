...
- **Prod DB was wiped clean on 2026-07-27** (sample data from the phase 4/5 verifications: contacts, lead lists, campaigns, steps, enrollments, messages, deals, stage changes, and the cyllabsdigital unsubscribe row). Deliberately kept: both Google-connected sending accounts with their tokens/app passwords, the `gmail.com` domain, and the 09:00–17:00 Australia/Sydney sending window. Pre-wipe dump is at `/root/crm-backups/cylrm-20260727T111252Z.sql` on the droplet. Prod schema is current as of the A/B variant work.
- Sending cadence (post-phase-7): the scheduler resumes `ooo_paused` enrollments once their `next_send_at` passes (bulk update at the top of the tick, reported as `resumedFromOoo`) — previously nothing did, so they stalled forever and stayed unenrollable. Due work is ordered follow-ups-first (`current_step = 0` ascending, so false sorts first): both touches share one daily cap, and a bulk enrollment stamps thousands of rows with an earlier `next_send_at` than any follow-up, which used to starve follow-ups until the backlog drained. `app_setting.send_weekdays_only` (default true) skips Sat/Sun judged in the sending timezone. `getCampaignProgress` counts `ooo_paused` in `remaining` (they resume, so the work is real) and converts capacity days to calendar days at 7/5 when weekends are off.
