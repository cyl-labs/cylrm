...
- Campaign progress (post-phase-7): campaign detail shows a progress card (sent vs owed, left to send, sent today, due now, estimated finish) plus a filterable enrollment list ordered by next send. Math lives in `src/lib/campaign-progress.ts`; the finish estimate is the later of the capacity bound (global — the account pool is shared across active campaigns) and the schedule bound (wait days still baked into the sequence).
...
- All phases 0–7 built. Outstanding: DO SMTP unblock (ticket #12611746) → then run the deferred Phase 3/4 live-send verifies; clear the prod test data (campaign 1 + enrollments/messages/deal + the cyllabsdigital unsubscribe row) before real campaigns. **Prod schema is behind local**: the NeverBounce drop and the A/B `variant` columns/enum have only been pushed to the local DB — run `drizzle-kit push` through the tunnel before the next deploy.
- **Known gap: OOO-paused enrollments never resume.** The poller sets `status = 'ooo_paused'` and pushes `next_send_at` out 7 days, but the scheduler only selects `status = 'active'` and nothing flips them back, so those contacts stall permanently. The blueprint intends them to resume. The progress card excludes them from "left to send" and warns instead. Fix needs a scheduler rule (reactivate `ooo_paused` once `next_send_at` passes) or a poller/cron sweep.
