...
- **No em dashes (—) in anything a person sees** (2026-09-25, founders: "makes
  our crm look cheap"). Screen text, toasts, errors, notifications, SOP
  markdown, and AI output (the brief prompt says so; `briefLines` strips old
  ones). Use a comma, colon, full stop or parentheses instead, a plain "-" for
  an empty cell, and watch for `&mdash;` as well as the character. Comments are
  exempt. When text follows an inline tag, write the space as `{" "}`.
...
- **The phone drawer folds the lesser screens** (2026-09-15, `NavLinks grouped`, `NAV_GROUPS` and `WorkspaceLink.group` in `lib/workspace.ts`). At fifteen Call CRM screens the list ran off an iPhone with Log out below it. Missed calls to Texts stay flat, since they are the work order and carry the badges, so **never put a badged link in a fold**. Below them sit Tools, Results and Admin, closed by default. Each lists what is inside when closed, and the fold holding the current page opens by itself. A fold with one link renders flat, which is a caller's Results (My stats only). The list scrolls on its own so Dark mode and Log out stay pinned, and only a link closes the drawer. **The desktop sidebar does not fold, and hides as a whole instead** (2026-09-23). It folded like the drawer for one day, for the crowding on the Founders login, and the founders asked for every link back: opening a fold on every visit cost more than the length did. The sidebar now collapses to a 64px rail of icons (`components/sidebar.tsx`, button at its top), saved per browser in the `cylrm-sidebar` cookie so the server renders the chosen width with no flash. Everything inside hides its words through `group-data-[collapsed=true]/sidebar:` variants, and badges shrink to a dot on the icon. **The workspace switcher is gone** from both the sidebar and the drawer: nobody moves between Call CRM and Email CRM, and the Email CRM's screens are still there by address.
- **A screen that takes more than about half a second gets a `loading.tsx`** rendering `PageLoading` (`components/page-loading.tsx`) with its own title (2026-09-24). It draws the page's header at once and a spinner that fades in after a beat, so the click visibly lands. The fast screens (about 0.3s) deliberately have none, since there it would only flash. It shows when you arrive at a screen, not when a filter on the same screen changes: Next keeps the old page up for that, so **`NavigationProgress`** (in the app layout) covers filters with a top bar, a "Loading…" label and a dimmed page body. Link clicks are caught automatically; **code that changes the address with `router.push`/`replace` must call `beginNavigation(href)` first**, or that control gives no sign it is working.
