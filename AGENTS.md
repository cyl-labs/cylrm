...
- The board's exclusion set is `OFF_BOARD` (`not_interested`, `bad_number`), **not** `TERMINAL`. TERMINAL also holds `interested` and `demo_booked` — they leave the *queue* but they are the last two columns of the *board*, and reusing it emptied both.
