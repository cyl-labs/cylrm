...
export async function listTeam(): Promise<TeamMember[]> {
  ...
  // Their lists are **not** fetched here. They were, in a second query, until
  // the Team screen started drawing a progress bar per list (2026-09-20): that
  // needs the retries and callbacks due as well, and counting those a second
  // way is how Team comes to disagree with the card on Call lists about the
  // same niche. `getCallLists` already answers all of it, so the Team page
  // calls that once and splits it by owner with `listsByOwner`. Stats, the
  // Scoreboard and Call lists — which never read the field — lost a query.
  return rows.map((r) => ({
    ...r,
    role: r.role,
