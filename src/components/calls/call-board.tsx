...
export function CallBoard({
  ...
  // Search and filters, applied in the browser over every card the page sent,
  // before the per-column cap — so a search finds a lead that would otherwise
  // sit past the first sixty in its column.
  const [query, setQuery] = React.useState("");
  const [lastCall, setLastCall] = React.useState<LastCallFilter>("any");
  // The moment the date filter was picked. Read in the handler rather than
  // while rendering, which must not look at the clock.
  const [pickedAt, setPickedAt] = React.useState(0);
  const [calledBy, setCalledBy] = React.useState("all");
  const callers = React.useMemo(
    () =>
      [...new Set(cards.map((c) => c.lastCalledBy).filter((n): n is string => !!n))].sort(
        (a, b) => a.localeCompare(b),
      ),
    [cards],
  );
  const filtering = query.trim() !== "" || lastCall !== "any" || calledBy !== "all";
  const filtered = React.useMemo(() => {
    ...
    return rows.filter((c) => {
      if (q) {
        ...
        if (!text.includes(q) && !byNumber) return false;
      }
      if (calledBy !== "all" && c.lastCalledBy !== calledBy) return false;
      if (lastCall === "never") return !c.lastCalledAt;
      if (lastCall !== "any") {
  ...
  return (
    <>
      <div className="flex shrink-0 flex-wrap items-center gap-2">
        <div className="relative w-full sm:w-80">
          ...
          <Input
            ...
            placeholder="Search business, name, number or notes"
        ...
        <Select
          value={lastCall}
          onValueChange={(v) => {
            setLastCall(v as LastCallFilter);
            setPickedAt(Date.now());
          }}
        >
          <SelectTrigger className="w-[calc(50%-0.25rem)] sm:w-56" aria-label="When they were last called">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {LAST_CALL.map((o) => (
        ...
        {/* Only worth offering when more than one person appears: on a caller's
            own board it would be a menu with their name in it. */}
        {callers.length > 1 && (
          <Select value={calledBy} onValueChange={setCalledBy}>
            <SelectTrigger className="w-[calc(50%-0.25rem)] sm:w-52" aria-label="Who made the last call">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Called by anyone</SelectItem>
              {callers.map((name) => (
        ...
        {filtering && (
          <>
            <button
              ...
              className="flex items-center gap-1 rounded-md px-2 py-1.5 text-[13px] font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
            >
            ...
            <span className="text-[13px] text-muted-foreground">
              {filtered.length === 0
                ? "No leads match. Try fewer words, or clear the filters."
                : `Showing ${filtered.length.toLocaleString()} of ${rows.length.toLocaleString()} leads`}
            </span>
          </>
        )}
      </div>
      ...
      >
        {columns.map((col) => {
          const all = filtered.filter((c) => c.stage === col.key);
              ...
              >
                ...
                {shown.length === 0 && (
                  <p className="px-2 py-6 text-center text-[12px] text-muted-foreground/70">
                    {filtering ? "Nothing here matches" : col.hint}
                  </p>
                )}
                {all.length > shown.length && (
...
type LastCallFilter = "any" | "today" | "7" | "30" | "older" | "never";

/** "Today" is the floor's day, the one Call lists and the Scoreboard count. */
const LAST_CALL: { value: LastCallFilter; label: string }[] = [
  { value: "any", label: "Last called: any time" },
  { value: "today", label: "Called today" },
  { value: "7", label: "Called in the last 7 days" },
  { value: "30", label: "Called in the last 30 days" },
  { value: "older", label: "Not called for over 30 days" },
